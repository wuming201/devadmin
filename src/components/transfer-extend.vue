...
export default {
  data() {
    return {
      ...
      to_check_keys: [],
      aaa: []// 目标数据选中key数组 以此属性关联穿梭按钮，总全选、半选状态
    };
  },
  props: {
    // 宽度
    ...
    addToAims() {
      ...
      newSkeletonHalfCheckedNodes.forEach(item => {
        ...
        if (item.parentId == 0) {
          ...
          this.$refs["to-tree"].append(item, item.parentId);
        }
      });
      ...
      console.log(cloneSkeletonCheckedNodes)
    ...
    removeToSource() {
      ...
      newSkeletonHalfCheckedNodes.forEach(item => {
        ...
        if (item.parentId == 0) {
          ...
          this.$refs["from-tree"].append(item, item.parentId);
        }
      });
  ...
  watch: {
    from_data() {
      this.self_from_data = this.from_data
    },
    // 左侧 状态监测
    from_check_keys(val) {
      if (val.length > 0) {
        ...
        let allCheck = val.filter(item => item.parentId == 0);
