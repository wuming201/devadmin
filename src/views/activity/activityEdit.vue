<template>
  <div class="activityInfo">
    <div class="memberInfo">
      <p><span><span class="title">活动ID:</span><span class="innerText">{{actInfo.id}}</span></span></p>
      <p><span><span class="title">活动标题:</span><span class="innerText"><el-input v-model="actInfo.title" placeholder="请输入内容"></el-input></span></span>
        <span><span class="title">活动类型:</span>
          <span class="innerText">
            <selectKuang :selectData="type" @value="actType"></selectKuang>
          </span>
        </span>
      </p>
      <p class="poster"><span class="mr0"><span class="title">活动海报:</span> <img :src="actInfo.img" alt=""><span class="imgBtn"><el-button type="primary" @click="upImg('changeImg')">替换图片</el-button>
      </span></span></p><el-input type="file" id="upimgs" v-on:change="upFile" hidden></el-input>
      <p><span class="mr0"><span class="title">活动说明:</span>
        <el-input
          type="textarea"
          :rows="2"
          placeholder="请输入内容"
          v-model="actInfo.explain">
        </el-input>
      </span></p>
      ...
      <p><span><span class="title">投递邮箱:</span><span class="innerText"><el-input v-model="address" placeholder="请输入内容"></el-input></span></span></p>
      ...
      <p><el-button type="success" @click="addAct">保存</el-button><el-button type="info" @click="quit">取消</el-button></p>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      addAct(){
        ...
        for (var i = 0; i<_this.gList.length; i++) {
          startTime.push(_this.gList[i]['sTime'])
          endTime.push(_this.gList[i]['eTime'])
        ...
        // this.getUEContent()
        // return
        PUBLIC.get( 'Active.actives.publish', {title: this.newTitle, body:this.newBody, img:this.changeImg}, function(data) {
          console.log(data)
          ...
          for(var n=0;n<_this.gList.length;n++){
              (function(id,data){
                console.log(data)
                PUBLIC.get('Active.actives.publigrop',{id: id, end_time: data.eTime, start_time: data.sTime, reg_end_time: data.rndTime, address: _this.address, reg_file: data.reg_file, money: data.price }, function(data){
                  groupFlagHad+=1
