<template>
  <div class="app-container">
    <div class="memberHead">
      ...
      <p><span class="buttons"><el-button type="success">批量导出</el-button><el-button type="danger" @click="showAdd">添加优惠券</el-button><el-button type="primary" @click="toHistory">使用记录</el-button><el-button type="warning" @click="tocoupHistory">历史优惠券</el-button></span>
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="已使用">
        <template slot-scope="scope">
          <span class="innerText">{{scope.row.sum - scope.row.had_num}}</span>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getDisList(keyword) {
        ...
        PUBLIC.get('Currency.receive.alldis', args, (data) => {
          // console.log(data)
          var newData = []
          var demo = {
            id: 'id',
      ...
      addDis() {
        ...
        let month = this.time.getMonth()+1    //js从0开始取
        let date1 = this.time.getDate()
        let hour = this.time.getHours()
        let minutes = this.time.getMinutes()
        let second = this.time.getSeconds()
        let time = year+ "-" +month+ "-" +date1+ " " +'0'+hour+ ":" + '0'+minutes + ":" +'0'+second
        console.log(time)
        let year1 = this.startTime.getFullYear()
        let month1 = this.startTime.getMonth()+1    //js从0开始取
        let date11 = this.startTime.getDate()
        let hour1 = this.startTime.getHours()
        let minutes1 = this.startTime.getMinutes()
        let second1 = this.startTime.getSeconds()
        let startTime = year1+ "-" +month1+ "-" +date11+ " " +'0'+hour1+ ":" + '0'+minutes1 + ":" +'0'+second1
        ...
        // }
        PUBLIC.get('Currency.receive.appenddis', { uid: 0, astype: 'shopping', type: 'discounts', item_id: this.months, reduce_num: this.money, had_num: this.had_num, time: time, startTime: startTime }, (data) => {
          console.log(data)
          _this.getDisList()
