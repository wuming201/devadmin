<template>
  <div class="app-container">
    <div class="memberHead">
      <p class="firstLine"><span class="phoneNum">消费内容：<selectKuang :selectData="payType" @value="payValue"></selectKuang></span>
        <span class="phoneNum">使用状态：<selectKuang :selectData="useState"  @value="stateValue"></selectKuang></span>
        <span class="phoneNum">支付方式：<selectKuang :selectData="payWay"  @value="wayValue"></selectKuang></span></p>
      <p><span class="buttons"><el-button type="success">批量导出</el-button></span>
        <span class="record">推广消费人次：<span>{{moneyCount.a}}<span>人次</span></span>推广消费金额：<span>{{moneyCount.b}}  <span>元</span></span>平台收益支出：<span>{{moneyCount.c}} <span>元</span></span><searchBox @searchKey='searchKey' v-bind:searchSelect='searchSelect'></searchBox></span></p>
    </div>
    <el-table
      ref="multipleTable"
      :data="tgData"
...
export default {
  ...
  methods: {
    ...
    getExtensionList(keyword) {
      ...
      PUBLIC.get('Order.receive.orderall', {commodity_type:this.commodity_type, money_code:1, page: this.page, select: JSON.stringify(args), order: 'id', order_aes: 1 }, function(data) {
        console.log(data)
        var demo = {
          id: 'id',
          uid: 'uid',
