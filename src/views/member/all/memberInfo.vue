<template>
  <div class="app-container">
    <el-container class="info clearfix">
      ...
      <el-main class="details"><span><span class="title">会员ID:</span><span class="innerText">{{uid}}</span></span><span><span
        class="title">用户名:</span><span class="innerText">{{username}}</span></span>
        <span><span class="title">绑定手机号:</span><span class="innerText">{{utelphone}}</span></span><span><span
          class="title">注册时间:</span><span class="innerText">{{ucreateTime}}</span></span>
        <span><span class="title">推广码:</span><span class="innerText">{{discount.key}}</span></span><span><span
          class="title">实名认证:</span><span class="innerText">{{urelStatus === '1'?'已实名':'未实名'}}</span></span>
        <span><span class="title">会员身份:</span><span class="innerText"><span class="iiText" v-for="items in userLevel">{{items}}</span>
        </span></span><span><span class="title">到期时间:</span><span class="innerText"><span class="iiText"
                                                                                          v-for="items in uendTime">{{items}}</span></span></span>
        <span><span class="title">收益比例:</span><span class="innerText">{{ugetMoney}}%</span></span><span><span
          class="title">积分:</span><span class="innerText">{{integral}}</span></span>
        <span class="lastp"><span class="title">账户余额:</span><span class="innerText">{{integral}}</span></span>
      </el-main>
      <el-aside width="140px" class="editBtn">
        <router-link :to="'/member/edit-member/' + uid">
    ...
    methods: {
      ...
      getMemberInfo: function (id) {
        ...
        PUBLIC.get('User.User.Userone', {uid: id}, (data) => {
          ...
          // 超管后台查询会员团队相关信息
          PUBLIC.get('Team.User.teaminfo', {uid: id}, (v) => {
            console.log(v)
            let newData = []
            let demo3 = {
              id: 'id',
            ...
            newData = PUBLIC.formatObj(demo3, v)
            this.teams = newData
